...
interface SendStatusDao {
    ...
    fun loadAllAsync(): LiveData<List<SendStatusEntity>>

    @Query("SELECT * FROM " + SendStatusEntity.TABLE_NAME + " WHERE send_id = :sendId ORDER BY created ASC")
    fun loadAllBySendIdAsync(sendId: String): LiveData<List<SendStatusEntity>>

    @Query("SELECT * FROM " + SendStatusEntity.TABLE_NAME + " WHERE id = :id")
    fun loadByIdAsync(id: String): LiveData<SendStatusEntity>

    @Query("SELECT * FROM " + SendStatusEntity.TABLE_NAME + " WHERE id = :id")
    fun loadById(id: String): SendStatusEntity?
